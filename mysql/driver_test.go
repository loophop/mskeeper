...
func init() {
	...
	dbname = env("MYSQL_TEST_DBNAME", "mskeepertestmysql")
...
func runDefaultPolicyWithOptionTests(t *testing.T, dsn string, tests ...func(dbt *DBTest)) {
	...
	// 	rawDB,
	// 	WithSwitch(true),
	// 	WithNotifier(notifier.NewNotifierMux(notifierUnitTest,
	...
	capa = options.FetchCapacity(msk.GetOptions())
...
func runDefaultPolicyWithOneCapacityTests(t *testing.T, dsn string, tests ...func(dbt *DBTest)) {
	...
	var capa int
	msk := MSKeeperInstance(dsn)
	// log.Printf("MSKeeperInstance of msk %v", msk)
	if msk == nil {
		t.Fatalf("failed to get driver for %v", db.Driver())
	}
	msk.ClearStatus()
	...
	log.Printf("msk.opts %v", msk.GetOptions())
	capa = options.FetchCapacity(msk.GetOptions())
...
func runTestsWithMultiStatement(t *testing.T, dsn string, tests ...func(dbt *DBTest)) {
	...
	if _, err := ParseDSN(dsn); err != errInvalidDSNUnsafeCollation {
		...
		msk.SetOptions(
			options.WithSwitch(true),
			options.WithNotifier(notifier.NewDefaultNotifier()),
		)
...
func runTests(t *testing.T, dsn string, tests ...func(dbt *DBTest)) {
	log.Printf("runTests on dsn %v", dsn)
	if !available {
		t.Skipf("MySQL server not running on %s", netAddr)
	}
	...
	if _, err := ParseDSN(dsn2); err != errInvalidDSNUnsafeCollation {
		db2, err = sql.Open("mskeeper", dsn2)
		if err != nil {
			t.Fatalf("error connecting: %s", err.Error())
		}
		defer db2.Close()

		msk := MSKeeperInstance(dsn2)
		// log.Printf("MSKeeperInstance of msk %v", msk)
		if msk == nil {
			t.Fatalf("failed to get driver for %v", db.Driver())
		}
	...
	if _, err := ParseDSN(dsn3); err != errInvalidDSNUnsafeCollation {
		db3, err = sql.Open("mskeeper", dsn3)
		if err != nil {
			t.Fatalf("error connecting: %s", err.Error())
		}
		defer db3.Close()

		msk := MSKeeperInstance(dsn3)
		// log.Printf("MSKeeperInstance of msk %v", msk)
		if msk == nil {
			t.Fatalf("failed to get driver for %v", db.Driver())
		}
...
func TestLoadData(t *testing.T) {
	...
	runTests(t, dsn, func(dbt *DBTest) {
		...
		// Then deregister the file, and try to load again
		DeregisterLocalFile(file.Name())
		_, err = dbt.db.Exec(fmt.Sprintf("LOAD DATA LOCAL INFILE %q INTO TABLE test", file.Name()))
		if err == nil {
			dbt.Fatal("load a file unregistered")
		}
		...
		// unregister the Reader, try again
		DeregisterReaderHandler("test")
		_, err = dbt.db.Exec("LOAD DATA LOCAL INFILE 'Reader::test' INTO TABLE test")
		if err == nil {
			dbt.Fatal("load non-registered reader didn't fail")
		}
		// negative test
...
func TestTLS(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("key 'skip-verify' should be reserved")
	}
	runTests(t, dsn+"&tls=skip-verify", tlsTestReq)

	err = RegisterTLSConfig("preferred", &tls.Config{
		InsecureSkipVerify: true,
	})
	if err == nil {
		t.Fatalf("key 'preferred' should be reserved")
	}
	runTests(t, dsn+"&tls=skip-verify", tlsTestReq)
...
func TestBadDSNWhenOpen(t *testing.T) {
	// this test does not use sql.database, it uses the driver directly
	newNetAddr := fmt.Sprintf("%s(%s)", "udp", "")
	if !available {
		t.Skipf("MySQL server not running on %s", newNetAddr)
	}

	badDsn := fmt.Sprintf("%s:%s@%s/%s?timeout=30s", user, pass, newNetAddr, dbname)
	md := &MSKeeperDriver{}
	_, err := md.Open(badDsn)
	if err == nil {
		t.Fatalf("error dsn parse: %v", err)
	}
	msk := MSKeeperInstance(badDsn)
	if msk != nil {
		t.Fatalf("error dsn parse got msk %v", msk)
	}

	_, err = md.OpenConnector(badDsn)
	if err == nil {
		t.Fatalf("error dsn parse: %v", err)
	}
}

func TestMSKeeperDriverOpenWhenNotInit(t *testing.T) {
	// this test does not use sql.database, it uses the driver directly
	if !available {
		t.Skipf("MySQL server not running on %s", netAddr)
	}

	// only diffs in server timeout, it should refer to a new msk instance
	newDsn := fmt.Sprintf("%s:%s@%s/%s?timeout=123s", user, pass, netAddr, dbname)
	msk := MSKeeperInstance(newDsn)
	if msk != nil {
		t.Fatalf("error in fetching msk %v", msk)
	}

	md := &MSKeeperDriver{}
	conn, err := md.Open(newDsn)
	if err != nil {
		t.Fatalf("error connecting: %s", err.Error())
	}
	err = conn.Close()
...
func TestTimezoneConversion(t *testing.T) {
	...
	tzTest := func(dbt *DBTest) {
		dbt.mustExec("DROP TABLE IF EXISTS test_timezone")

		// Create table
		dbt.mustExec("CREATE TABLE test_timezone (ts TIMESTAMP)")
		...
		dbt.mustExec("INSERT INTO test_timezone VALUE (?)", reftime)

		// Retrieve time from DB
		rows := dbt.mustQuery("SELECT ts FROM test_timezone")
		defer rows.Close()
		if !rows.Next() {
...
func testDialError(t *testing.T, dialErr error, expectErr error) {
	...
	db, err := sql.Open("mskeeper", fmt.Sprintf("%s:%s@mydial(%s)/%s?timeout=30s", user, pass, addr, dbname))
	if err != nil {
		t.Fatalf("error connecting: %s", err.Error())
	}
	defer db.Close()
...
func TestCustomDial(t *testing.T) {
	...
	RegisterDial("mydialdepre", func(addr string) (net.Conn, error) {
		var d net.Dialer
		return d.Dial(prot, addr)
	})

	db, err = sql.Open("mskeeper", fmt.Sprintf("%s:%s@mydialdepre(%s)/%s?timeout=30s", user, pass, addr, dbname))
	if err != nil {
		t.Fatalf("error connecting: %s", err.Error())
	}
	defer db.Close()
...
func TestUnixSocketAuthFail(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		...
		db, err := sql.Open("mskeeper", badDSN)
		if err != nil {
			t.Fatalf("error connecting: %s", err.Error())
		}
		defer db.Close()
...
func TestRowsColumnTypes(t *testing.T) {
	// nsNULL := sql.NullString{String: "", Valid: true}
	...
	rbg1 := sql.RawBytes("\x00\x00\x00\x00\x01\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00")
	rbg2 := sql.RawBytes("\x00\x00\x00\x00\x01\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\xF0\x3F\x00\x00\x00\x00\x00\x00\xF0\x3F")
	rbg3 := sql.RawBytes("\x00\x00\x00\x00\x01\x01\x00\x00\x00\x87\x88\x9B\x53\xC9\xC0\xF3\x3F\xE1\x5D\x2E\xE2\x3B\xDD\x5E\x40")

	var columns = []struct {
		name             string
		fieldType        string    // type used when creating table schema
		databaseTypeName string    // actual type used by MySQL
		msFieldType      fieldType // represent mysql field type
		...
		{"bit8null", "BIT(8)", "BIT", fieldTypeBit, scanTypeRawBytes, true, 0, 0, [3]string{"0x0", "NULL", "0x42"}, [3]interface{}{rbx0, rbNULL, rbx42}},
		{"boolnull", "BOOL", "TINYINT", fieldTypeTiny, scanTypeNullInt, true, 0, 0, [3]string{"NULL", "true", "0"}, [3]interface{}{niNULL, ni1, ni0}},
		{"bool", "BOOL NOT NULL", "TINYINT", fieldTypeTiny, scanTypeInt8, false, 0, 0, [3]string{"1", "0", "FALSE"}, [3]interface{}{int8(1), int8(0), int8(0)}},
		{"intnull", "INTEGER", "INT", fieldTypeLong, scanTypeNullInt, true, 0, 0, [3]string{"0", "NULL", "42"}, [3]interface{}{ni0, niNULL, ni42}},
		{"smallint", "SMALLINT NOT NULL", "SMALLINT", fieldTypeShort, scanTypeInt16, false, 0, 0, [3]string{"0", "-32768", "32767"}, [3]interface{}{int16(0), int16(-32768), int16(32767)}},
		{"smallintnull", "SMALLINT", "SMALLINT", fieldTypeShort, scanTypeNullInt, true, 0, 0, [3]string{"0", "NULL", "42"}, [3]interface{}{ni0, niNULL, ni42}},
		{"int3null", "INT(3)", "INT", fieldTypeLong, scanTypeNullInt, true, 0, 0, [3]string{"0", "NULL", "42"}, [3]interface{}{ni0, niNULL, ni42}},
		{"int7", "INT(7) NOT NULL", "INT", fieldTypeLong, scanTypeInt32, false, 0, 0, [3]string{"0", "-1337", "42"}, [3]interface{}{int32(0), int32(-1337), int32(42)}},
		{"mediumintnull", "MEDIUMINT", "MEDIUMINT", fieldTypeInt24, scanTypeNullInt, true, 0, 0, [3]string{"0", "42", "NULL"}, [3]interface{}{ni0, ni42, niNULL}},
		{"bigint", "BIGINT NOT NULL", "BIGINT", fieldTypeLongLong, scanTypeInt64, false, 0, 0, [3]string{"0", "65535", "-42"}, [3]interface{}{int64(0), int64(65535), int64(-42)}},
		{"bigintnull", "BIGINT", "BIGINT", fieldTypeLongLong, scanTypeNullInt, true, 0, 0, [3]string{"NULL", "1", "42"}, [3]interface{}{niNULL, ni1, ni42}},
		{"tinyuint", "TINYINT UNSIGNED NOT NULL", "TINYINT", fieldTypeTiny, scanTypeUint8, false, 0, 0, [3]string{"0", "255", "42"}, [3]interface{}{uint8(0), uint8(255), uint8(42)}},
		{"smalluint", "SMALLINT UNSIGNED NOT NULL", "SMALLINT", fieldTypeShort, scanTypeUint16, false, 0, 0, [3]string{"0", "65535", "42"}, [3]interface{}{uint16(0), uint16(65535), uint16(42)}},
		{"biguint", "BIGINT UNSIGNED NOT NULL", "BIGINT", fieldTypeLongLong, scanTypeUint64, false, 0, 0, [3]string{"0", "65535", "42"}, [3]interface{}{uint64(0), uint64(65535), uint64(42)}},
		{"uint13", "INT(13) UNSIGNED NOT NULL", "INT", fieldTypeLong, scanTypeUint32, false, 0, 0, [3]string{"0", "1337", "42"}, [3]interface{}{uint32(0), uint32(1337), uint32(42)}},
		{"float", "FLOAT NOT NULL", "FLOAT", fieldTypeFloat, scanTypeFloat32, false, math.MaxInt64, math.MaxInt64, [3]string{"0", "42", "13.37"}, [3]interface{}{float32(0), float32(42), float32(13.37)}},
		{"floatnull", "FLOAT", "FLOAT", fieldTypeFloat, scanTypeNullFloat, true, math.MaxInt64, math.MaxInt64, [3]string{"0", "NULL", "13.37"}, [3]interface{}{nf0, nfNULL, nf1337}},
		{"float74null", "FLOAT(7,4)", "FLOAT", fieldTypeFloat, scanTypeNullFloat, true, math.MaxInt64, 4, [3]string{"0", "NULL", "13.37"}, [3]interface{}{nf0, nfNULL, nf1337}},
		{"double", "DOUBLE NOT NULL", "DOUBLE", fieldTypeDouble, scanTypeFloat64, false, math.MaxInt64, math.MaxInt64, [3]string{"0", "42", "13.37"}, [3]interface{}{float64(0), float64(42), float64(13.37)}},
		{"doublenull", "DOUBLE", "DOUBLE", fieldTypeDouble, scanTypeNullFloat, true, math.MaxInt64, math.MaxInt64, [3]string{"0", "NULL", "13.37"}, [3]interface{}{nf0, nfNULL, nf1337}},
		{"decimal1", "DECIMAL(10,6) NOT NULL", "DECIMAL", fieldTypeDecimal, scanTypeRawBytes, false, 10, 6, [3]string{"0", "13.37", "1234.123456"}, [3]interface{}{sql.RawBytes("0.000000"), sql.RawBytes("13.370000"), sql.RawBytes("1234.123456")}},
		{"decimal1null", "DECIMAL(10,6)", "DECIMAL", fieldTypeDecimal, scanTypeRawBytes, true, 10, 6, [3]string{"0", "NULL", "1234.123456"}, [3]interface{}{sql.RawBytes("0.000000"), rbNULL, sql.RawBytes("1234.123456")}},
		{"decimal2", "DECIMAL(8,4) NOT NULL", "DECIMAL", fieldTypeDecimal, scanTypeRawBytes, false, 8, 4, [3]string{"0", "13.37", "1234.123456"}, [3]interface{}{sql.RawBytes("0.0000"), sql.RawBytes("13.3700"), sql.RawBytes("1234.1235")}},
		{"decimal2null", "DECIMAL(8,4)", "DECIMAL", fieldTypeDecimal, scanTypeRawBytes, true, 8, 4, [3]string{"0", "NULL", "1234.123456"}, [3]interface{}{sql.RawBytes("0.0000"), rbNULL, sql.RawBytes("1234.1235")}},
		{"decimal3", "DECIMAL(5,0) NOT NULL", "DECIMAL", fieldTypeDecimal, scanTypeRawBytes, false, 5, 0, [3]string{"0", "13.37", "-12345.123456"}, [3]interface{}{rb0, sql.RawBytes("13"), sql.RawBytes("-12345")}},
		{"decimal3null", "DECIMAL(5,0)", "DECIMAL", fieldTypeDecimal, scanTypeRawBytes, true, 5, 0, [3]string{"0", "NULL", "-12345.123456"}, [3]interface{}{rb0, rbNULL, sql.RawBytes("-12345")}},
		{"char25null", "CHAR(25)", "CHAR", fieldTypeString, scanTypeRawBytes, true, 0, 0, [3]string{"0", "NULL", "'Test'"}, [3]interface{}{rb0, rbNULL, rbTest}},
		{"varchar42", "VARCHAR(42) NOT NULL", "VARCHAR", fieldTypeVarChar, scanTypeRawBytes, false, 0, 0, [3]string{"0", "'Test'", "42"}, [3]interface{}{rb0, rbTest, rb42}},
		{"binary4null", "BINARY(4)", "BINARY", fieldTypeString, scanTypeRawBytes, true, 0, 0, [3]string{"0", "NULL", "'Test'"}, [3]interface{}{rb0pad4, rbNULL, rbTest}},
		{"varbinary42", "VARBINARY(42) NOT NULL", "VARBINARY", fieldTypeVarChar, scanTypeRawBytes, false, 0, 0, [3]string{"0", "'Test'", "42"}, [3]interface{}{rb0, rbTest, rb42}},
		{"tinyblobnull", "TINYBLOB", "BLOB", fieldTypeTinyBLOB, scanTypeRawBytes, true, 0, 0, [3]string{"0", "NULL", "'Test'"}, [3]interface{}{rb0, rbNULL, rbTest}},
		{"tinytextnull", "TINYTEXT", "TEXT", fieldTypeTinyBLOB, scanTypeRawBytes, true, 0, 0, [3]string{"0", "NULL", "'Test'"}, [3]interface{}{rb0, rbNULL, rbTest}},
		{"blobnull", "BLOB", "BLOB", fieldTypeBLOB, scanTypeRawBytes, true, 0, 0, [3]string{"0", "NULL", "'Test'"}, [3]interface{}{rb0, rbNULL, rbTest}},
		{"textnull", "TEXT", "TEXT", fieldTypeBLOB, scanTypeRawBytes, true, 0, 0, [3]string{"0", "NULL", "'Test'"}, [3]interface{}{rb0, rbNULL, rbTest}},
		{"mediumblob", "MEDIUMBLOB NOT NULL", "BLOB", fieldTypeMediumBLOB, scanTypeRawBytes, false, 0, 0, [3]string{"0", "'Test'", "42"}, [3]interface{}{rb0, rbTest, rb42}},
		{"mediumtext", "MEDIUMTEXT NOT NULL", "TEXT", fieldTypeMediumBLOB, scanTypeRawBytes, false, 0, 0, [3]string{"0", "'Test'", "42"}, [3]interface{}{rb0, rbTest, rb42}},
		{"longblob", "LONGBLOB NOT NULL", "BLOB", fieldTypeLongBLOB, scanTypeRawBytes, false, 0, 0, [3]string{"0", "'Test'", "42"}, [3]interface{}{rb0, rbTest, rb42}},
		{"longtext", "LONGTEXT NOT NULL", "TEXT", fieldTypeLongBLOB, scanTypeRawBytes, false, 0, 0, [3]string{"0", "'Test'", "42"}, [3]interface{}{rb0, rbTest, rb42}},
		{"datetime", "DATETIME", "DATETIME", fieldTypeDateTime, scanTypeNullTime, true, 0, 0, [3]string{"'2006-01-02 15:04:05'", "'2006-01-02 15:04:05.1'", "'2006-01-02 15:04:05.111111'"}, [3]interface{}{nt0, nt0, nt0}},
		{"datetime2", "DATETIME(2)", "DATETIME", fieldTypeDateTime, scanTypeNullTime, true, 2, 2, [3]string{"'2006-01-02 15:04:05'", "'2006-01-02 15:04:05.1'", "'2006-01-02 15:04:05.111111'"}, [3]interface{}{nt0, nt1, nt2}},
		{"datetime6", "DATETIME(6)", "DATETIME", fieldTypeDateTime, scanTypeNullTime, true, 6, 6, [3]string{"'2006-01-02 15:04:05'", "'2006-01-02 15:04:05.1'", "'2006-01-02 15:04:05.111111'"}, [3]interface{}{nt0, nt1, nt6}},
		{"date", "DATE", "DATE", fieldTypeDate, scanTypeNullTime, true, 0, 0, [3]string{"'2006-01-02'", "NULL", "'2006-03-04'"}, [3]interface{}{nd1, ndNULL, nd2}},
		{"newdate", "DATE", "DATE", fieldTypeNewDate, scanTypeNullTime, true, 0, 0, [3]string{"'2006-01-02'", "NULL", "'2006-03-04'"}, [3]interface{}{nd1, ndNULL, nd2}},
		{"year", "YEAR NOT NULL", "YEAR", fieldTypeYear, scanTypeUint16, false, 0, 0, [3]string{"2006", "2000", "1994"}, [3]interface{}{uint16(2006), uint16(2000), uint16(1994)}},
		{"geometry", "GEOMETRY NOT NULL", "GEOMETRY", fieldTypeGeometry, scanTypeRawBytes, false, 0, 0, [3]string{"X'00000000010100000000000000000000000000000000000000'", "X'000000000101000000000000000000F03F000000000000F03F'", "X'00000000010100000087889B53C9C0F33FE15D2EE23BDD5E40'"}, [3]interface{}{rbg1, rbg2, rbg3}},
		{"geometrynull", "GEOMETRY", "GEOMETRY", fieldTypeGeometry, scanTypeRawBytes, true, 0, 0, [3]string{"X'00000000010100000000000000000000000000000000000000'", "NULL", "NULL"}, [3]interface{}{rbg1, rbNULL, rbNULL}},
		{"set", "SET('stone','Test','machine','egg') NOT NULL", "CHAR", fieldTypeString, scanTypeRawBytes, false, 0, 0, [3]string{"'Test'", "'egg'", "''"}, [3]interface{}{rbTest, sql.RawBytes("egg"), sql.RawBytes("")}},
		{"setnull", "SET('stone','Test','machine','egg')", "CHAR", fieldTypeString, scanTypeRawBytes, true, 0, 0, [3]string{"NULL", "'egg'", "NULL"}, [3]interface{}{rbNULL, sql.RawBytes("egg"), rbNULL}},
		{"enum", "enum('sichuang','shanghai') NOT NULL", "CHAR", fieldTypeString, scanTypeRawBytes, false, 0, 0, [3]string{"'sichuang'", "'shanghai'", "''"}, [3]interface{}{sql.RawBytes("sichuang"), sql.RawBytes("shanghai"), sql.RawBytes("")}},
		{"enumnull", "enum('sichuang','shanghai')", "CHAR", fieldTypeString, scanTypeRawBytes, true, 0, 0, [3]string{"NULL", "'shanghai'", "NULL"}, [3]interface{}{rbNULL, sql.RawBytes("shanghai"), rbNULL}},
		{"time", "TIME NOT NULL", "TIME", fieldTypeTime, scanTypeRawBytes, false, 0, 0, [3]string{"'15:04:05'", "'15:04:05.1'", "'15:04:05.111111'"}, [3]interface{}{sql.RawBytes("15:04:05"), sql.RawBytes("15:04:05"), sql.RawBytes("15:04:05")}},
		{"time2", "TIME(2) NOT NULL", "TIME", fieldTypeTime, scanTypeRawBytes, false, 2, 2, [3]string{"'15:04:05'", "'15:04:05.1'", "'15:04:05.111111'"}, [3]interface{}{sql.RawBytes("15:04:05.00"), sql.RawBytes("15:04:05.10"), sql.RawBytes("15:04:05.11")}},
		{"time6", "TIME(6) NOT NULL", "TIME", fieldTypeTime, scanTypeRawBytes, false, 6, 6, [3]string{"'15:04:05'", "'15:04:05.1'", "'15:04:05.111111'"}, [3]interface{}{sql.RawBytes("15:04:05.000000"), sql.RawBytes("15:04:05.100000"), sql.RawBytes("15:04:05.111111")}},
		{"timenull", "TIME", "TIME", fieldTypeTime, scanTypeRawBytes, true, 0, 0, [3]string{"NULL", "'15:04:05.1'", "''"}, [3]interface{}{rbNULL, sql.RawBytes("15:04:05"), sql.RawBytes("00:00:00")}},
		{"timestamp", "TIMESTAMP NOT NULL", "TIMESTAMP", fieldTypeTimestamp, scanTypeNullTime, false, 0, 0, [3]string{"'2006-01-02'", "'2006-01-02 15:04:05'", "'2006-03-04'"}, [3]interface{}{nd1, nt0, nd2}},
	}
	...
	for _, testdsn := range dsns {
		runTests(t, testdsn, func(dbt *DBTest) {
			...
			log.Print("CREATE TABLE test (" + schema + ")")

			insertStr := "INSERT INTO test VALUES (" + values1 + "), (" + values2 + "), (" + values3 + ")"
			log.Printf("insertStr: %v", insertStr)
			dbt.mustExec(insertStr)
			rows, err := dbt.db.Query("SELECT * FROM test")
			if err != nil {
			...
			for i, tp := range tt {
				...
				log.Printf("column(%v) : %v, tp %v databaseTypeName %v column.databaseTypeName %v",
					i, column, tp, databaseTypeName, column.databaseTypeName)
				...
				// field type
				var charSet uint8
				fts := strings.Split(databaseTypeName, " ")
				databaseTypeNameInMSK := strings.Split(fts[0], "(")[0]
				if databaseTypeNameInMSK == "BLOB" ||
					databaseTypeNameInMSK == "TINYBLOB" ||
					databaseTypeNameInMSK == "MEDIUMBLOB" ||
					databaseTypeNameInMSK == "LONGBLOB" ||
					databaseTypeNameInMSK == "BINARY" ||
					databaseTypeNameInMSK == "VARBINARY" {
					charSet = collations[binaryCollation]
				}

				msField := mysqlField{fieldType: column.msFieldType, charSet: charSet}
				if databaseTypeNameInMSK != msField.typeDatabaseName() {
					t.Errorf("databasetypename name mismatch for column %q: %s != %s", name, databaseTypeNameInMSK, msField.typeDatabaseName())
					continue
				}
			}
				...
				for j := range values {
					...
					if !reflect.DeepEqual(value, columns[j].valuesOut[i]) {
						if columns[j].scanType == scanTypeRawBytes {
							log.Printf("value=%v, value %T, columns[j].valuesOut[i]=%v, columns[j].valuesOut[i]=%T",
								value.(sql.RawBytes), value, columns[j].valuesOut[i], columns[j].valuesOut[i])
...
var _ driver.DriverContext = &MSKeeperDriver{}
...
func TestConnectorObeysDialTimeouts(t *testing.T) {
	...
	db, err := sql.Open("mskeeper", fmt.Sprintf("%s:%s@dialctxtest(%s)/%s?timeout=30s", user, pass, addr, dbname))
	if err != nil {
		t.Fatalf("error connecting: %s", err.Error())
	}
	defer db.Close()
...
func TestBinaryRowsReadRow(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {

		dbt.mustExec("CREATE TABLE test (id int, value1 tinyint, value2 smallint, value3 tinyint unsigned, value4 smallint unsigned, value5 MEDIUMINT, value6 MEDIUMINT unsigned, value7 time(6), value8 datetime(6))")
		dbt.mustExec("INSERT INTO test VALUES (1, 127, 32767, 255, 65535, 8388607, 16777215, '15:04:05.123456', '2006-01-02 15:04:05.111111')")
		rows := dbt.mustQuery("SELECT value1, value2, value3, value4, value5, value6, value7, value8 FROM test WHERE id = ?", 1)
		defer rows.Close()
		if rows.Next() {
			...
			if err != nil {
				dbt.Fatalf("rows.Scanf failed %v", err)
			}
			if v1 != 127 {
				dbt.Fatalf("v1 %v != 127", v1)
			}
			if v2 != 32767 {
				dbt.Fatalf("v1 %v != 32767", v2)
			}
			if v3 != 255 {
				dbt.Fatalf("v3 %v != 255", v3)
			}
			if v4 != 65535 {
				dbt.Fatalf("v4 %v != 65535", v4)
			}
			if v5 != 8388607 {
				dbt.Fatalf("v5 %v != 8388607", v5)
			}
			if v6 != 16777215 {
				dbt.Fatalf("v6 %v != 16777215", v6)
			}
			if v7 != "15:04:05.123456" {
				dbt.Fatalf("v7 %v != 15:04:05.123456", v7)
			}
			if v8 != "2006-01-02 15:04:05.111111" {
				dbt.Fatalf("v8 %v != 2006-01-02 15:04:05.111111", v8)
			}
		} else {
			dbt.Errorf("no data")
		}
