	...
	runDefaultPolicyWithOneCapacityTests(t, dsn, func(dbt *DBTest) {
		...
		for i := 0; i < 100000; i++ {
			...
			go func(idx int) {
				...
				dbt.mustExec("INSERT INTO testaddon VALUES (?, ?)", idx, idx)
				wg.Done()
			}(i)
		}
		...
		if err == nil && (int(secs) < driver.MaxTimeoutSecondsForFlush || int(secs) > driver.MaxTimeoutSecondsForFlush+1) {
			dbt.Errorf("flush not timed out with MaxTimeoutSecondsForFlush %v actual :%0.1vs ",
				driver.MaxTimeoutSecondsForFlush, secs)
		}
...
func TestPolicyafterFlushTimeoutFail(t *testing.T) {
	runDefaultPolicyWithOneCapacityTests(t, dsn, func(dbt *DBTest) {
		...
		row := dbt.mustQueryRow("SELECT count(1) from testaddon ")
...
func TestPolicyafterFlushTimeoutSkip(t *testing.T) {
	runDefaultPolicyWithOneCapacityTests(t, dsn, func(dbt *DBTest) {
		...
		for i := 0; i < 3; i++ {
			...
			go func(idx int) {
				// dbt.db.Exec("select 1 from testaddon where value = '1'")
				dbt.mustExec(fmt.Sprintf("INSERT INTO testaddon VALUES (%v, %v)", idx, idx))
				wg.Done()
				log.Print("Done")
			}(i)
		}
