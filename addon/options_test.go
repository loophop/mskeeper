...
func TestPolicyOptionsSwitchInProcess(t *testing.T) {
	runDefaultPolicyWithOptionTests(t, dsn+"&columnsWithAlias=true", func(dbt *DBTest) {
		...
		for i := 0; i < 10; i++ {
			...
			dbt.mustQueryRow("select * from testaddon where testaddon.value in (select value from testaddon) and testaddon.value = ?;", i)
...
func TestPolicyOptionsMaxSilentPeriod(t *testing.T) {
	runDefaultPolicyWithOptionTests(t, dsn+"&columnsWithAlias=true", func(dbt *DBTest) {
		...
		notifierUnitTest.ClearErr()
		dbt.mustQueryRow("insert into testaddon select * from testaddon where testaddon.value in (select value from testaddon);")
		dbt.db.Flush()
		if !notifierUnitTest.HasErr(policy.ErrPolicyCodeRowsAbs) {
			dbt.Errorf("rows absolutely not covered")
		}
...
func TestPolicyOptionsLogLevelTests(t *testing.T) {
	runDefaultPolicyWithOptionTests(t, dsn+"&columnsWithAlias=true", func(dbt *DBTest) {
		...
		// 每条SQL分析，有两条Info日志
		if nsw.Count() != 30 {
			dbt.Errorf("nsw.Count() %v != 30+1", nsw.Count())
		}
		...
		for i := 0; i < 10; i++ {
			...
			dbt.db.QueryRow("select xxxvalue from testaddon where 13123123=testaddon.value and value = ?", i)
		}
		dbt.db.Flush()

		if nsw.Count() != 30+10 {
			dbt.Errorf("nsw.Count() %v != 30+10 %v", nsw.Count(), nsw.GetLogs())
		}
...
func TestPolicyOptionsSQLWhiteList(t *testing.T) {
	runDefaultPolicyWithOptionTests(t, dsn+"&columnsWithAlias=true", func(dbt *DBTest) {
		...
		// 加入了白名单，应该被忽略
		dbt.db.SetOption(options.WithSQLWhiteLists("select * from testaddon where testaddon.value in (select value from testaddon) and testaddon.value = ?;"))
		dbt.mustQueryRow("select * from testaddon where testaddon.value in (select value from testaddon) and testaddon.value = ?;", 1)
		...
		// 没有加入白名单，应该被检测出来
		dbt.mustQueryRow("select value from testaddon where testaddon.value in (select value from testaddon) and testaddon.value = ?;", 1)
		dbt.db.Flush()
		if notifierUnitTest.WithNoErrs() {
			dbt.Errorf("sql whitelist not work properly in process routine of mskeeper")
		}
