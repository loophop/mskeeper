...
func TestPolicyExcutionTime2(t *testing.T) {
	runDefaultPolicyTests(t, dsn+"&columnsWithAlias=true", func(dbt *DBTest) {
		...
		dbt.mustExec("insert into testaddon select * from testaddon where testaddon.value1 in (select value1 from testaddon);")
		dbt.db.Flush()
		if notifierUnitTest.HasErr(policy.ErrPolicyCodeExeCost) {
...
func TestPolicyRowsInvolved(t *testing.T) {
	runDefaultPolicyTests(t, dsn+"&columnsWithAlias=true", func(dbt *DBTest) {
		...
		dbt.mustExec("CREATE TABLE `testaddon` ( `value` int(11) DEFAULT NULL, `value1` int(11) DEFAULT NULL, KEY `value1` (`value1`) ) ENGINE=InnoDB DEFAULT CHARSET=utf8;")
		for i := 0; i < 3004; i++ {
			dbt.mustExec("INSERT INTO testaddon VALUES (?, ?)", i, i)
		}
		dbt.db.Flush()
		...
		dbt.mustQueryRow("select * from testaddon where value1 < 20 limit 1")
		dbt.db.Flush()
		...
		dbt.mustQueryRow("select * from testaddon where value1 < 1005")
		dbt.db.Flush()
		if !notifierUnitTest.HasErr(policy.ErrPolicyCodeRowsInvolve) {
		...
		dbt.mustQueryRow("select * from testaddon where value > 20")
		dbt.db.Flush()
		if !notifierUnitTest.HasErr(policy.ErrPolicyCodeRowsInvolve) {
...
func TestPolicyRowsAbsolute(t *testing.T) {
	runDefaultPolicyTests(t, dsn+"&columnsWithAlias=true", func(dbt *DBTest) {
		...
		dbt.mustQueryRow("select * from testaddon where testaddon.value in (select value from testaddon);")
		dbt.db.Flush()
		log.Printf("dbt.db.GetErr() %v", dbt.db.GetErr())
		if !notifierUnitTest.HasErr(policy.ErrPolicyCodeRowsAbs) {
			dbt.Errorf("rows absolutely not covered")
...
func TestPolicyFieldsTypeSimple(t *testing.T) {
	runDefaultPolicyTests(t, dsn+"&columnsWithAlias=true", func(dbt *DBTest) {
		...
		dbt.db.SetOption(options.WithMaxExecTime(3 * time.Second))
		dbt.mustExec("CREATE TABLE `testaddon` ( `value` int(11), `value1` varchar(60), KEY `idx_value` (`value`), KEY `idx_value1` (`value1`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;")
		for i := 0; i < 1010; i++ {
			dbt.mustExec("INSERT INTO testaddon VALUES (?, ?)", i, i)
		}
		dbt.db.Flush()
		...
		rows := dbt.mustQuery("select * from testaddon where value1 = ?", godamnv)
		defer rows.Close()
		dbt.db.Flush()
...
func TestPolicyFieldsTypeUnionOK(t *testing.T) {
	runDefaultPolicyTests(t, dsn+"&columnsWithAlias=true", func(dbt *DBTest) {
		...
		rows := dbt.mustQuery("select * from testaddon where testaddon.value1 = '111' union select * from testaddon where testaddon.value1 = '222';")
		defer rows.Close()
		dbt.db.Flush()
...
func TestPolicyFieldsLengthStringOK1(t *testing.T) {
	runDefaultPolicyTests(t, dsn+"&columnsWithAlias=true", func(dbt *DBTest) {
		...
		dbt.mustExec("CREATE TABLE `testaddon` ( `value` int(11), `value1` varchar(60), KEY `idx_value` (`value`), KEY `idx_value1` (`value1`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;")
		for i := 0; i < 1001; i++ { // 超过1000行
			dbt.mustExec("INSERT INTO testaddon VALUES (?, ?)", i, fmt.Sprintf("%v", i))
		}
		dbt.db.Flush()
		...
		if _, err := tx.Exec("INSERT INTO testaddon VALUES(?, ?)", 11111, "222222"); err != nil {
			tx.Rollback()
			t.Errorf("TestPolicyFieldsLengthStringOK1: transaction exec insert failed")
		}
		if _, err := tx.Exec("UPDATE testaddon SET value1 = ? WHERE value = ? ", "222222", 22222); err != nil {
			tx.Rollback()
			t.Errorf("TestPolicyFieldsLengthStringOK1: transaction exec insert failed")
		}
...
func TestPolicyFieldsLengthStringOK2(t *testing.T) {
	runDefaultPolicyTests(t, dsn+"&columnsWithAlias=true", func(dbt *DBTest) {
		...
		dbt.db.SetOption(options.WithMaxExecTime(3 * time.Second))
		dbt.mustExec("CREATE TABLE `testaddon` ( `value` int(11), `value2` varchar(5), `value1` varchar(60), KEY `idx_value` (`value`), KEY `idx_value1` (`value1`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;")
		for i := 0; i < 1001; i++ { // 超过1000行
			dbt.mustExec("INSERT INTO testaddon VALUES (?, ?, ?)", i, "1234", i)
		}
		dbt.db.Flush()
...
func TestPolicyFieldsLengthStringNG1(t *testing.T) {
	runDefaultPolicyTests(t, dsn+"&columnsWithAlias=true", func(dbt *DBTest) {
		...
		dbt.mustExec("CREATE TABLE `testaddon` ( `value` int(11), `value2` varchar(5), `value1` varchar(60), KEY `idx_value` (`value`), KEY `idx_value1` (`value1`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;")
		for i := 0; i < 1001; i++ { // 超过1000行
			dbt.mustExec("INSERT INTO testaddon VALUES (?, ?, ?)", i, "123456", i+1)
		}
		dbt.db.Flush()
